...
- View the directories currently stored in the `PATH` variable
  ```bash
    echo $PATH
  ```
  - **Function:** The `PATH` variable controls where the system looks for executable commands (like `cd`, `ls`, or `echo`).
  - **Structure:** Each directory path is separated by a colon (`:`).
  - **Syntax Note:** You must add the `$` symbol before `PATH` to view its contents (value) rather than the word "PATH".
...
- Demonstrate the incorrect command to assign `/root/newhackingtool` to the `PATH` variable and explain why this leads to a "command not found" error for standard system commands
  ...
  - **Result:** The shell cannot locate standard commands (like `cd` or `ls`), causing them to fail unless you navigate directly to their physical location.
